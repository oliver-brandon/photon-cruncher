...
def validate_repo(values: dict[str, str]) -> list[str]:
    ...
    if values["update_package_id"] != values["bundle_identifier"]:
        errors.append("update_package_id and bundle_identifier must match")
    if not values["update_channel"].startswith("stable-"):
        errors.append("update_channel must remain isolated to stable releases")
    if values["update_release_tag"] != f"v{values['version']}":
    ...
    required_text = {
        ...
        PROJECT_ROOT / "photon_cruncher" / "main.py": (
            "run_velopack_startup()",
        ),
        PROJECT_ROOT / "photon_cruncher" / "gui" / "main_window.py": (
            "UpdateController(self)",
        ),
        PROJECT_ROOT / "photon_cruncher" / "gui" / "updater.py": (
            "Check for Updates…",
            "6 * 60 * 60 * 1000",
            "Install and restart",
        ),
        ...
        PROJECT_ROOT / "packaging" / "velopack" / "release-notes.md": (
            f"# Photon Cruncher {values['version']}",
        ),
    }
    ...
    for path in literal_targets:
        ...
        for literal in forbidden:
            if literal in text:
                errors.append(
                    f"{path.relative_to(PROJECT_ROOT)} hard-codes canonical "
                    f"value {literal!r}"
                )
    return errors
