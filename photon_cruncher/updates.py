"""Velopack update service for stable Photon Cruncher installations."""
...
class UpdateConfigurationError(RuntimeError):
    """Raised when an updater could escape the stable release boundary."""
...
def validate_stable_channel(channel: str, expected: str | None = None) -> str:
    ...
    if not channel.startswith(f"{UPDATE_CHANNEL_PREFIX}-"):
        raise UpdateConfigurationError(
            f"Refusing non-stable update channel {channel!r}"
        )
...
class VelopackBackend:
    ...
    def __init__(
        ...
        *,
        ...
        self.channel = validate_stable_channel(channel or update_channel())
        self.package_id = package_id
        source = velopack.GithubSource(repository_url, None, False)
...
class UpdateService:
    ...
    def _get_backend(self) -> UpdateBackend:
        if self._backend is None:
            ...
            validate_stable_channel(backend.channel, self.channel)
    ...
    def check(self) -> UpdateSnapshot:
        self._set(
            UpdateSnapshot(
                ...
                self.channel,
                message="Checking for Photon Cruncher updates...",
            )
        )
        try:
            ...
            if release is None or not is_newer_version(
                ...
                return self._set(
                    UpdateSnapshot(
                        ...
                        self.current_version,
                        self.channel,
                        message="Photon Cruncher is up to date.",
                    )
                )
            return self._set(
                UpdateSnapshot(
                    ...
                    self.channel,
                    release=release,
                    message=f"Photon Cruncher v{release.version} is available.",
                )
            )
            ...
            return self._set(
                UpdateSnapshot(
                    ...
                    self.channel,
                    message=(
                        "Could not reach the Photon Cruncher update service. "
