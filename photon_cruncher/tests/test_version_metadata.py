...
from photon_cruncher import __version__, app_title
...
class VersionMetadataTests(unittest.TestCase):
    ...
    def test_stable_update_identity_derives_from_canonical_version(self) -> None:
        channel, runtime = update_target("Darwin", "arm64")
        self.assertEqual(channel, f"{UPDATE_CHANNEL_PREFIX}-{runtime}")
        self.assertEqual(UPDATE_PACKAGE_ID, "com.photoncruncher.app")
        self.assertEqual(UPDATE_RELEASE_TAG, f"v{__version__}")

    def test_friendly_names_do_not_change_stable_update_identity(self) -> None:
        ...
        self.assertNotIn("aurora-dev", mac_script + windows_script + workflow)

    def test_repository_version_metadata_is_consistent(self) -> None:
        env = {
            **os.environ,
            "PHOTON_CRUNCHER_UPDATE_SYSTEM": "Windows",
            "PHOTON_CRUNCHER_UPDATE_MACHINE": "AMD64",
        }
